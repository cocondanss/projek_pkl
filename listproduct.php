...
<div class="container-index" style="max-width: 100%;">
    ...
    $isUsed = isVoucherUsed($voucherCode);
    $discountedPrice = $isUsed ? $originalPrice : applyVoucher($voucherCode, $originalPrice);
?>
    <div class="product" data-product-id="<?php echo $item['id']; ?>">
        <div class="card-body"> 
            <h2><?php echo htmlspecialchars($item['name']); ?></h2>
            <div class="price-container">
                <?php if ($discountedPrice < $originalPrice && !$isUsed): ?>
                ...
                <?php else: ?>
                    ...
                    <?php if ($isUsed): ?>
                        <p class="voucher-used-message">Voucher sudah pernah digunakan</p>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
            <p><?php echo htmlspecialchars($item['description']); ?></p>
            <button onclick="showPaymentModal(<?php echo $item['id']; ?>, '<?php echo htmlspecialchars($item['name']); ?>', <?php echo number_format($originalPrice, 0, '', ''); ?>)">
                Buy
            </button>
        </div>
    </div>
