...
if (isset($_POST['TambahTransaksi'])) {
    ...
    $productPrice = applyVoucher($voucherCode, $price); // Terapkan voucher jika ada

    // Simpan transaksi ke database
    $order_id = 'TRX-' . time() . '-' . uniqid(); 
    $status = ($productPrice == 0) ? 'completed' : 'pending'; // Set status berdasarkan harga

    $addtotable = mysqli_query($conn, "INSERT INTO transaksi (order_id, product_name, price, status) VALUES ('$order_id', '$product_name', '$productPrice', '$status')");
...
if (isset($_SESSION['message'])) {
    echo '<div class="alert alert-success">' . htmlspecialchars($_SESSION['message']) .
    '</div>';
...
$addtotable = mysqli_query($conn, "INSERT INTO products (name, deskripsi, price, id, content) VALUES ('$name', '$deskripsi', '$price', '$id', '$content')");
...
'" download="' . $fileName . '">Unduh File</a>';
exit;
}
